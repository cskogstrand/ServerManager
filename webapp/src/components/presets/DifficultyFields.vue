...
// Difficulty preset form body. Shared by the Difficulty page and the inline
// "create preset" sheet. v-model is the preset (nullable until loaded).
...
const form = defineModel<UserDifficulty | null>({ required: true });
...
const trackPresets = ["Custom", "Dusty", "Old", "Slow", "Green", "Fast", "Optimum"].map((label, value) => ({
  value,
  label,
}));
</script>
