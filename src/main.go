...
func main() {
	...
	if err := reconcileDrivingExecutions(); err != nil {
		log.Fatal("Execution recovery failed: ", err)
	}
	...
	api.Use(pitlaneLifecycleMiddleware)
	{
		api.GET("/driving-sessions", apiDrivingSessions)
		api.GET("/driving-sessions/defaults", apiDrivingDefaults)
		api.GET("/driving-sessions/template/:id", apiDrivingTemplate)
		api.POST("/driving-sessions/preview", apiDrivingPreview)
		api.GET("/driving-sessions/:id", apiDrivingSession)
		api.GET("/driving-sessions/:id/recap", apiDrivingRecap)
		api.GET("/driving-sessions/:id/results/:result", apiExecutionResult)
		api.POST("/driving-sessions", apiDrivingSessionSave)
		api.PUT("/driving-sessions/:id", apiDrivingSessionSave)
		api.POST("/driving-sessions/:id/start", apiDrivingSessionLaunch)
		api.POST("/driving-sessions/:id/finish", apiDrivingFinish)
		api.GET("/rigs", apiRigs)
		api.POST("/rigs", apiRigSave)
		api.PUT("/rigs/:id", apiRigSave)
		api.DELETE("/rigs/:id", apiRigDelete)
		api.GET("/sources", apiCameraSources)
		api.POST("/sources", apiCameraSourceSave)
		api.PUT("/sources/:key", apiCameraSourceSave)
		api.GET("/sources/status", apiCameraSourceHealth)
		api.POST("/sources/:key/capture/:action", apiSourceCapture)
		api.GET("/sources/:key/jobs", apiSourceCaptureJobs)
		api.GET("/sources/:key/media", apiSourceMedia)
		api.GET("/sources/:key/media/:file", apiSourceMedia)
		api.DELETE("/sources/:key/media/:file", apiSourceMediaMutate)
		api.POST("/sources/:key/media/:file/assign", apiSourceMediaMutate)
		api.POST("/sources/:key/check", apiCameraSourceHealth)
		...
		api.GET("/content/metadata", apiContentMetadataList)
		api.GET("/content/metadata/:kind/:key", apiContentMetadata)
		api.PUT("/content/metadata/:kind/:key", apiContentMetadataSave)
		api.GET("/recovery/cleanup", apiCleanupPreview)
		api.POST("/recovery/cleanup", apiCleanupApply)
		api.GET("/recovery/changes", apiMetadataChanges)
		api.POST("/recovery/changes/:id/undo", apiMetadataUndo)
		...
		api.GET("/results/files", apiResultFiles)
		...
		api.DELETE("/drivers/:guid/sessions/:id", apiDriverSessionDelete)
