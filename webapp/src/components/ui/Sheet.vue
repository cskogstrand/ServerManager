  ...
  <Teleport to="body">
    <dialog ref="dialog" :aria-labelledby="titleId" class="app-dialog app-sheet" @cancel.prevent="emit('close')" @click.self="emit('close')">
      <div v-if="open" class="flex h-dvh w-full flex-col border-l border-line bg-surface text-text shadow-2xl" :class="wide ? 'sm:max-w-5xl' : 'sm:max-w-md'">
        ...
        <div class="min-h-0 flex-1 overflow-y-auto bg-bg p-5 sm:p-6"><slot /></div>
        <footer v-if="$slots.footer" class="flex flex-wrap items-center justify-end gap-2 border-t border-line bg-rail px-6 pt-4 pb-[max(1rem,env(safe-area-inset-bottom))]"><slot name="footer" /></footer>
      </div>
    </dialog>
  </Teleport>
</template>
