...
function sourceDriver(source){return state.drivers.find(d=>d.id===(source?.id==='rig-01'?state.seat:source?.driver));}
...
function setupStreams(){const ws=watchState();modal('Your simulators, connected.',`<p>Link a feed once. Watch it here and from any session its driver joins.</p><div class="stream-manager">${ws.sources.map(source=>`<div class="managed-source"><span class="setting-icon">${icon('screen')}</span><span><b>${e(source.name)}</b><small>${e(sourceName(source))} · ${sourceHealth(source)}</small></span><button class="button compact" data-watch-action="source-settings" data-value="${source.id}" aria-label="Edit ${e(source.name)}">Edit</button></div>`).join('')}</div><div class="dialog-actions"><button class="button primary" data-watch-action="new-source">${icon('plus')} Add simulator or camera</button></div><p class="notice">Demo sources only. Playback, recording, and driver presence have separate statuses.</p>`);}
function sourceEditor(id,driver=''){
  const source=getSource(id),next=source||{name:'',driver,status:'unknown',url:'',capture:'',auto:false};
  modal(source?'Set up '+e(source.name):'Connect a simulator.',`<p>A name and a player link are enough to watch. Recording is optional.</p><form data-watch-form="source" data-source-id="${e(id||'')}" data-checked="false"><label class="field">Simulator or camera name<input name="name" value="${e(next.name)}" placeholder="Rig 05" required maxlength="50"></label><label class="field">Player URL<input name="url" type="url" value="${e(next.url)}" placeholder="https://your-stream-server/rig-05/whep" required><small>WebRTC / WHEP or an embeddable player page.</small></label><label class="field">Who uses this simulator?<select name="driver" ${source?.id==='rig-01'?'disabled':''}><option value="">No driver assigned / spectator camera</option>${state.drivers.map(d=>`<option value="${d.id}" ${d.id===(source?.id==='rig-01'?state.seat:next.driver)?'selected':''}>${e(d.name)}</option>`).join('')}</select>${source?.id==='rig-01'?`<input type="hidden" name="driver" value="${e(state.seat)}">`:''}<small>${source?.id==='rig-01'?'Use Next driver in Session control to hand over this shared rig. Its camera stays linked.':'The feed follows this driver into the session they join.'}</small></label><label class="field spectator-session" ${next.driver?'hidden':''}>Session for an unassigned camera<select name="session"><option value="">None · available in the live wall</option>${state.sessions.filter(s=>s.status==='live').map(s=>`<option value="${s.id}" ${s.id===next.session?'selected':''}>${e(s.name)}</option>`).join('')}</select><small>Optional. Use this for a trackside or spectator view.</small></label>
...
function sourceValues(form){const f=new FormData(form);return {name:String(f.get('name')||'').trim(),url:String(f.get('url')||'').trim(),driver:String(f.get('driver')||''),session:f.get('driver')?'':String(f.get('session')||''),capture:String(f.get('capture')||'').trim(),auto:f.get('auto')==='on'};}
...
function invalidateSourceCheck(event){const form=event.target.closest('[data-watch-form="source"]');if(!form)return;if(event.target.name==='driver')form.querySelector('.spectator-session').hidden=!!event.target.value;form.dataset.checked='false';form.querySelector('[type="submit"]').disabled=true;form.querySelector('.source-check').textContent='Check the sample connection after making changes.';}
